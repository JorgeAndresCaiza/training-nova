...
# MAGIC %md-sandbox
# MAGIC <image src="https://storagepocdemos.blob.core.windows.net/public/images/databricks_banner.png?sp=r&st=2025-09-07T16:20:01Z&se=2027-01-01T04:59:59Z&spr=https&sv=2024-11-04&sr=b&sig=93gWaOconS54rPMBkuUQ5hq9ji6GVlLPofBBZeI7yYI%3D" widht="100%" alt="handytec"></image>
...
CSV_URL="https://storagepocdemos.blob.core.windows.net/public/documents/insurance_sample.csv?sp=r&st=2025-09-08T20:22:14Z&se=2027-01-01T04:59:59Z&spr=https&sv=2024-11-04&sr=b&sig=uUEG5%2FuqAjs1VVs4ayO7O8%2F2hDViE%2BKGqbNJZlA2Kr0%3D"
 
df_pandas = pd.read_csv(CSV_URL, sep=',', header='infer')
...
path = "/Volumes/dbw_edwh_training/db_bronze/input_files_dp/Ejemplo01.csv"
df = spark.read.csv(path = path, header=True, encoding="UTF-8",inferSchema= True, sep= ",", multiLine= True )
display(df)

# COMMAND ----------

path = "/Volumes/dbw_edwh_training/db_bronze/input_files_dp/plantilla_parametros 1.xlsx"
pandas_df = pd.read_excel(path,sheet_name='plantilla_parametros')
display(pandas_df)

# COMMAND ----------

grant select on any file to capacitacion
...
json_df_aux = json_df.select(explode(json_df.data).alias("data")).selectExpr("data.*","data.billingAddress.*", "now()")
...
# MAGIC %md
# MAGIC # Jason desde campo string Usando Esquema
...
table_name = "dbw_edwh_training.db_bronze.product_items_DP"
...
# MAGIC %sql
# MAGIC SELECT * FROM dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC DROP TABLE dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC DELETE FROM dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE HISTORY dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC RESTORE TABLE dbw_edwh_training.db_bronze.product_items_DP TO VERSION AS OF 0

# COMMAND ----------

# MAGIC %sql
# MAGIC VACUUM dbw_edwh_training.db_bronze.product_items_DP

# COMMAND ----------

# MAGIC %sql
# MAGIC OPTIMIZE dbw_edwh_training.db_bronze.product_items_DP
